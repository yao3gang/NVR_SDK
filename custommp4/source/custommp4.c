...
custommp4_t* custommp4_open(char *filename, u8 open_mode, u8 write_mode, u32 open_offset)
{
	custommp4_t *file;
	char flags[10];
	...
	else
	{
		...
		if(file->write_mode == WRITE_WITHOUT_BUF)
		{
		...
		if (file->write_mode == WRITE_WITH_BUF)
		{
...
int custommp4_write_video_frame(custommp4_t *file,u8 *video_buffer,u32 bytes,u32 timestamp,u8 isKeyFrame, u32 frameID, u8 *update)
{
	video_frame_info_table_t tmp_vframe;
	u32 last_pos;

 	if (!isKeyFrame)//if not a key frame
	{
		if (file->b_wait_key)//if waiting a key frame
		{
			return 0;
		}
		else if ((file->last_frame_id + 1) != frameID)//check if a frame is lost
		{
			file->b_wait_key = 1;
			return 0;
		}
...
int custommp4_write_video_frame_with_buf(custommp4_t *file, u8 *video_buffer, u32 bytes, u32 timestamp, u8 isKeyFrame, u32 frameID, u64 pts,u8 *update, char *p_w_buf, u32 *p_w_buf_len, u32 maxlen)
{
	u32 last_pos;
	//printf("w v f 1\n");
	...
	//printf("w v f tmp offset =%d,v f=%d,allocated=%d\n",tmp_offset,file->p_mdat_sect_t->video_frames , file->video_table_allocated);
	//printf("w v f 8.1  %d\n",file->p_video_table_t[file->p_mdat_sect_t->video_frames].offset);
	//printf("w v f 8.1.0\n");
	...
	if (p_w_buf && p_w_buf_len)
	{
		...
		{
		//printf("w v f 12\n");
			custommp4_write_data(file, p_w_buf, *p_w_buf_len);
			memcpy(p_w_buf, video_buffer, bytes);
			*p_w_buf_len = bytes;
		}
		else
		{
