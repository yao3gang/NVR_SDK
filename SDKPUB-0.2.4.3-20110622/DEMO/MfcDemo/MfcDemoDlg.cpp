...
BOOL CMfcDemoDlg::OnInitDialog()
{
	...
	GetDlgItem(IDC_ServerAddr)->SetWindowText("192.168.1.46");
	GetDlgItem(IDC_ServerPort)->SetWindowText("6650");
...
void CALLBACK deal_frame_record(pFrameHeadr pFrmHdr, unsigned int dwContent)
{
	...
	if ((cur_length + pFrmHdr->dataSize) > ((128<<20) - (1<<19)))
	{
		if (pThis->file)
		{
	...
	if (b_video)
	{
		...
		{
			if (b_wait_key)//if waiting a key frame
			{
				return;
			}
			else if ((last_frame_id + 1) != pFrmHdr->frameID)//check if a frame is lost
			{
				b_wait_key = TRUE;
				return;
			}
...
void CMfcDemoDlg::OnStartRec() 
{
	...
	for(int i = 0; i < 1; i++)
	{
		...
		ret = NETDVR_GetAudioProperty(m_RealHandle[i], &m_AudioPro);
...
void CMfcDemoDlg::OnRecSearch() 
{
	...
	if (NETDVR_SUCCESS == ret)
	{
		...
		p_FileInfo = rec_desc.precInfo;
		CString szInfo;
		szInfo.Format("FileSearch successfully!\n startID = %d, endID = %d, sum = %d\n",
			rec_desc.startID, rec_desc.endID, rec_desc.sum);
		for (int i=0; i<min(rec_desc.sum, rfs.max_return); i++)
		{
			CString szFile;
			szFile.Format("name:%s, offset:%d, size:%d\n", 
			p_FileInfo->filename, p_FileInfo->offset, p_FileInfo->size);
			szInfo += szFile;
			p_FileInfo = p_FileInfo->pnext;
		}
...
void CMfcDemoDlg::OnUpdate() 
{
	...
	update_para.reserved = NETDVR_UPDATE_PANEL;//NETDVR_UPDATE_MOTHERBBOARD;
	
	CFile file;
	BOOL bRet = file.Open("c:\\panel.hex", CFile::modeRead);
	if (!bRet)
	{
		AfxMessageBox("Open File c:\\panel.hex failed !");
		return;
	}
	...
	ret = NETDVR_startUpdate(m_hDvr, &update_para, "c:", "panel.hex"); //
	if (NETDVR_SUCCESS != ret)
	{
...
{
	AfxMessageBox("this sample will set connect time to 5000 ms!", MB_ICONINFORMATION);
	int ret = NETDVR_setConnectTimeout(5000);
}
...
int g_serialnPort = 1; //1-232  2-485
